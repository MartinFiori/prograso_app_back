...
async function create(req, res) {
  const data = await eventCategoriesService.create(req.body, req.file ?? null)
  const body = new ApiSuccess({ data, statusCode: httpStatusCodes.CREATED })
  res.status(body.statusCode).json(body)
}

async function update(req, res) {
  const data = await eventCategoriesService.update(req.params.id, req.body, req.file ?? null)
  const body = new ApiSuccess({ data })
  res.status(body.statusCode).json(body)
}
